...
public class PartFactory {
    ...
    public void setRigidBodyProperty(String key, Object value) {
        if (!RB_PROPERTIES.containsKey(key))
            ...
            throw new IllegalArgumentException("Invalid RigidBodyProperty value for mesh, " + value +
                    " is not of type PhysicsMesh");
    ...
    private void finishPolygon(GObject obj, List<PVector> vertices) {
        ...
        Primitive primitive = new Primitive(rigidBody, surface, vertices);
    ...
    private void finishCircle(GObject obj, float radius) {
        RigidBody rigidBody = new RigidBody(obj, new RigidBodyProperties(RB_PROPERTIES));
        Surface surface = new Surface(restitution, staticFriction, dynamicFriction);
        Primitive primitive = new Primitive(rigidBody, surface, radius);
        //TODO how to assign RigidBody and Primitive to GObject?
    }


    public RObject createRect(GObject parent, PVector anchor, PVector dimensions, float orientation, LColour colour, int id) {
    ...
    private GObject createRect(GObject obj, PVector dimensions, float orientation) {
        obj.addVisualAttributes(VisualUtils.makeRect(
                dimensions, DEFAULT_COLOUR));
    ...
    public RObject createTriangle(GObject parent, PVector anchor, PVector vertex1, PVector vertex2, LColour colour, int id) {
    ...
    private GObject createTriangle(GObject obj, PVector vertex1, PVector vertex2) {
        obj.addVisualAttributes(VisualUtils.makeTriangle(
                new PVector(), vertex1.copy(), vertex2.copy(), DEFAULT_COLOUR));
    ...
    public RObject createPolygon(GObject parent, PVector anchor, PVector dimensions, int sides, float orientation, LColour colour, int id) {
    ...
    private GObject createPolygon(GObject obj, PVector dimensions, int sides, float orientation) {
        obj.addVisualAttributes(VisualUtils.makeUntexturedPolygon(
                dimensions, sides, orientation, new PVector(), DEFAULT_COLOUR));
        ...
        for (int i = 0; i < sides; i++) {
            vertices.add(new PVector(
                    (float) (dimensions.x / 2 * Math.cos(angle)),
    ...
    public RObject createEllipse(GObject parent, PVector anchor, PVector dimensions, float orientation, LColour colour, int id) {
    ...
    private GObject createCircle(GObject obj, float radius) {
        obj.addVisualAttributes(VisualUtils.makeUntexturedPolygon(
                new PVector(radius, radius), ELLIPSE_VERTICES, 0f, new PVector(), DEFAULT_COLOUR));
    ...
    // TODO
    public RObject makeLever(GObject parent, PVector anchor, float orientation, int[] linkedIDs, LColour colour, int id) {
        return null;
    }
